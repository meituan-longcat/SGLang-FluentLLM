...
class MooncakeKVManager(BaseKVManager):
    ...
    def send_aux(
        ...
        # Transfer all aux buffers (output_ids, logprobs, cached_tokens, etc.)
        src_addrs = []
        dst_addrs = []
        lengths = []
        for i, (aux_data_ptr, aux_item_len, dst_aux_ptr) in enumerate(
            zip(self.kv_args.aux_data_ptrs, self.kv_args.aux_item_lens, dst_aux_ptrs)
        ):
        ...
        if status != 0:
            logger.error(f"Failed to transfer aux buffer: status={status}")
            return status
        return 0
    ...
    def add_transfer_request(
        ...
        logger.debug(f"manager add transfer_request")
        if (
...
class MooncakeKVReceiver(BaseKVReceiver):
    ...
    def __init__(
        ...
        local_tp_size_per_dp_rank = self.kv_mgr.world_size // self.kv_mgr.dp_size
